 ...
 *
 * Prereqs:
 *  - Install Python deps once: `cd services/bot-python && python3 -m venv .venv && source .venv/bin/activate && pip install -r requirements.txt`
 *
 * Options via env:
...
async function verifyUvicorn(pythonBin: string) {
...
function waitOn(proc: any, name: string) {
...
function runVite() {
  const p = Bun.spawn(['bun', 'vite', '--port', String(FRONTEND_PORT), '--strictPort'], {
    cwd: rootCwd,
    stdout: 'inherit',
    stderr: 'inherit',
    env: { ...process.env }
